...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	calories := ((runningCaloriesMeanSpeedMultiplier * avSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return calories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	avSpeedINsec := meanSpeed(action, duration) * kmhInMsec
	calories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(avSpeedINsec, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return calories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	avSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)
	calories := (avSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return calories
